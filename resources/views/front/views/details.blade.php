...
@section('title', "Mouse Famous Fundraising Indonesia")
...
@section('content')
    ...
    <section class="max-w-[640px] w-full min-h-screen mx-auto flex flex-col bg-white overflow-x-hidden">
        ...
        <div class="flex flex-col z-30">
            ...
            <div id="content" class="w-full bg-white rounded-t-[40px] flex flex-col gap-5 p-[30px_24px_120px]">
                <div class="flex flex-col gap-[10px]">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="w-9 h-9 flex shrink-0 rounded-full overflow-hidden">
                            <img src="{{Storage::url($fundraising->fundraiser->user->avatar)}}" class="w-full h-full object-cover" alt="photo">
                        </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <progress id="fund" value="{{$fundraising->getPercentage()}}" max="100" class="w-full h-[6px] rounded-full overflow-hidden"></progress>
                </div>
        ...
        @if(!$goalReached)
        <a href="{{route('front.support', $fundraising)}}" class="p-[14px_20px] bg-[#76AE43] rounded-full text-white w-fit mx-auto font-semibold hover:shadow-[0_12px_20px_0_#76AE4380] transition-all duration-300 fixed bottom-[30px] transform -translate-x-1/2 left-1/2 z-40 text-nowrap">Send My Support Now</a>
        @endif
    </section>
